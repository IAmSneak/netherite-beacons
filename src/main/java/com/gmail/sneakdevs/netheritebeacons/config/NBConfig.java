...
public class NBConfig implements ConfigData {
    ...
    public static NBConfig getInstance() {
        return AutoConfig.getConfigHolder(NBConfig.class).getConfig();
    }

    public static float getBonus(StatusEffect se) {
        if (StatusEffects.HASTE.equals(se)) {
            return AutoConfig.getConfigHolder(NBConfig.class).getConfig().hasteBonus;
        } else if (StatusEffects.SPEED.equals(se)) {
            return AutoConfig.getConfigHolder(NBConfig.class).getConfig().speedBonus;
        } else if (StatusEffects.RESISTANCE.equals(se)) {
            return AutoConfig.getConfigHolder(NBConfig.class).getConfig().resistanceBonus;
        } else if (StatusEffects.JUMP_BOOST.equals(se)) {
            return AutoConfig.getConfigHolder(NBConfig.class).getConfig().jumpBoostBonus;
        } else if (StatusEffects.STRENGTH.equals(se)) {
            return AutoConfig.getConfigHolder(NBConfig.class).getConfig().strengthBonus;
        }
        return 0.5F;
    }
}